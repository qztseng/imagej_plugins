 ...
 */
public class FourierMellinRegistration_ implements PlugIn {
    ...
    public void run(String arg) {
		...
		if(subpixel){
            ...
            double[] dxdyG1 = gaussianPeakFit(result, dxdy[0], dxdy[1]);
			double[] dxdyG2 = gaussianPeakFit(result, dxdy[2], dxdy[3]);

			rot1 = dxdyG1[1]*360f/size_result;
			rot2 = dxdyG2[1]*360f/size_result;
		}else{
			rot1 = dxdy[1]*360f/size_result;
			rot2 = dxdy[3]*360f/size_result;
		}
		...
		//ImageStack stk = new ImageStack(size_result, size_result);
	...
	public FloatProcessor transformFM(ImagePlus img, int cHigh, int cLow){
		
		return transformFM(img.getProcessor(), cHigh, cLow);
	}
	
	public FloatProcessor transformFM(ImageProcessor ip, int cHigh, int cLow){
    ...
    private boolean getParams() {
        ...
        gd.addNumericField("Highpass filter threshold ", cutoffHigh, 0);   //as in imageJ built-in find maxima
        gd.addNumericField("Lowpass filter threshold ", cutoffLow, 0);   //minimum value for a maxima
	...
	public FloatProcessor HLpassFiltering(ImageProcessor ip, int cHigh, int cLow){
	...
	private ImageProcessor getPaddedIP(ImagePlus img){
	...
	private FloatProcessor calcFilter(int N, int cHigh, int cLow){
	...
	private FloatProcessor Gaussian(int N, int cutoff, boolean high){
	...
	private FloatProcessor calculateAmplitude(float[] fhtArray, int maxN) {
	...
	public FloatProcessor xcorr(ImageProcessor ip1, ImageProcessor ip2){
	...
	*/
	...
	/*
	* Gaussian peak fit, the input x, and y are mid-point substrated coordinates
	* The output will also be the mid-point substracted coordinates
	*/
	private double[] gaussianPeakFit(ImageProcessor ip, int x0, int y0) {
        double[] coord = new double[2];
        
		int x = x0+ip.getWidth()/2;
		int y = y0+ip.getHeight()/2;
		...
		double[] coord_sub = new double[2];
		coord_sub[0] = coord[0]-ip.getWidth()/2;
		coord_sub[1] = coord[1]-ip.getHeight()/2;
