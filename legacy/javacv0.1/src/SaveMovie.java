        ...
        try {
            ...
            if(dim[3]!=1){
                ...
                if(dim[4]!=1){
                    ...
                    IJ.showMessage("higher dimension stack not supported, please reduce either depth or time");
                    return DONE;
                }
            }  
			if(dim[0]%2 !=0 || dim[1]%2 !=0 ){   //some codec will have problem encoding image frame with dimensions not divisible by 2
				IJ.showMessage("some codec (H264) will have problem encoding image frame with dimensions not divisible by 2");
			}
        ...
        if(speed){
            try {
                ...
                for (int i = 1; i <= frames; i++) {
					...
					//image = Java2DFrameUtils.toIplImage(bi);
                    recorder.record(new Java2DFrameConverter().convert(bi), AV_PIX_FMT_ARGB);   //BGRA
                    //image.release();
                    bi.flush();
                }
                recorder.stop();
            ...
            try {
                ...
                for (int i = 1; i <= stk.getSize(); i++) {
                    ...
                    //image = IplImage.createFrom(bi);
					//image = Java2DFrameUtils.toIplImage(bi);
                    recorder.record(new Java2DFrameConverter().convert(bi), AV_PIX_FMT_ARGB);
                    //image.release();
                    bi.flush();
                }
                recorder.stop();
